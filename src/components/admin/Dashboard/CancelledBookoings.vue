<template>
  <div   style="width: 400px; height: auto; margin: 0 auto; ">
   <h1>Cancelled Bookings</h1>
    <div v-for="(booking,key) in bookings" :key="key" class="p-3 " style="background-color: mediumpurple; border: 1px solid black">
...
export default {
  ...
  methods:{
    async getCancelledBookings(){
      await this.axios.get('getCancelledBookings').then(response=>{
        this.bookings = response.data.cancelled
